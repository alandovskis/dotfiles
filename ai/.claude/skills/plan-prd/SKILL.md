...
*Always run all three (6A SDD, 6B Test Plan, 6C Implementation Plan).*

Read `references/document-templates.md` now — it has the exact document structure for each of the three assembled documents, plus the Test Plan's type-to-section reorganization rule. Fill in its bracketed placeholders from the sections/task groups/test cases produced in Step 5; do not alter its structure.
...
- **`references/document-templates.md`** — exact assembly templates for the SDD, Test Plan, and Implementation Plan (Step 6)
