...
If the user invoked this skill with a feature or product name in their message, use it as context. Then use the host's user-interaction mechanism to collect the following in batches (max 4 questions per call):
...
Use a fresh reviewer agent or isolated review context when the host supports delegation. Pass the draft PRD explicitly and give the reviewer only the material below. If delegation is unavailable, perform the reviewer pass yourself without relying on generation reasoning. The reviewer evaluates only the supplied draft.
